...
export class SnowtraceDefinition extends SingleIntegrationDefinition {
  integrationKey = 'snowtrace'
    ...
    for (const pathKey of Object.keys(schema.paths)) {
      ...
      for (const key of replaceKeys) {
        if (schema.paths[pathKey].get[key]) {
          schema.paths[pathKey].get[key] = schema.paths[pathKey].get[key].replace('Ether ', 'AVAX ')
        }
      }
    }
    return schema
  ...
  requestInterceptor({ req, credentials }: RequestInterceptorOptions): OptionsWithUrl {
    if (req.url) {
      req.url += `&apikey=${credentials.token ?? process.env.SNOWTRACE_KEY}`
    }
    return req
  }
}
