  ...
  updateSchemaBeforeSave(schema: OpenAPIObject): Promise<OpenAPIObject> {
    ...
    for (const [pathKey, pathValue] of Object.entries(schema.paths)) {
      for (const [operationKey, operationValue] of Object.entries(pathValue)) {
        const isWhitelisted = (operationValue as OperationObject).security?.[0].slackAuth.some((scope) => {
          return (
            !scope.includes('admin') && (scope === 'none' || botScopes.some((botScope) => scope.includes(botScope)))
          )
        })
        if (!isWhitelisted) {
    ...
    // @ts-expect-error
    schema.components.securitySchemes.slackAuth.flows.authorizationCode.scopes = botScopes.reduce((prev, curr) => {
      prev[curr] = curr
      return prev
    }, {})
