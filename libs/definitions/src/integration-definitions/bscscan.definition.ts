...
export class BscscanDefinition extends SingleIntegrationDefinition {
  integrationKey = 'bscscan'
    ...
    for (const pathKey of Object.keys(schema.paths)) {
      ...
      for (const key of replaceKeys) {
        if (schema.paths[pathKey].get[key]) {
          schema.paths[pathKey].get[key] = schema.paths[pathKey].get[key].replace('Ether ', 'BNB ')
        }
      }
    }
    return schema
  ...
  requestInterceptor({ req, credentials }: RequestInterceptorOptions): OptionsWithUrl {
    if (req.url) {
      req.url += `&apikey=${credentials.token ?? process.env.BSCSCAN_KEY}`
    }
    return req
  }
}
