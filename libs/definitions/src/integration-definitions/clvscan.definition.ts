...
export class ClvscanDefinition extends SingleIntegrationDefinition {
  integrationKey = 'clvscan'
    ...
    for (const pathKey of Object.keys(schema.paths)) {
      ...
      for (const key of replaceKeys) {
        if (schema.paths[pathKey].get[key]) {
          schema.paths[pathKey].get[key] = schema.paths[pathKey].get[key].replace('Ether ', 'CLV ')
        }
      }
    }
    return schema
  ...
  requestInterceptor({ req, credentials }: RequestInterceptorOptions): OptionsWithUrl {
    if (req.url) {
      req.url += `&apikey=${credentials.token ?? process.env.CLVSCAN_KEY}`
    }
    return req
  }
}
