...
export class GnosisscanDefinition extends SingleIntegrationDefinition {
  integrationKey = 'gnosisscan'
    ...
    for (const pathKey of Object.keys(schema.paths)) {
      ...
      for (const key of replaceKeys) {
        if (schema.paths[pathKey].get[key]) {
          schema.paths[pathKey].get[key] = schema.paths[pathKey].get[key].replace('Ether ', 'xDAI ')
        }
      }
    }
    return schema
  ...
  requestInterceptor({ req, credentials }: RequestInterceptorOptions): OptionsWithUrl {
    if (req.url) {
      req.url += `&apikey=${credentials.token ?? process.env.GNOSISSCAN_KEY}`
    }
    return req
  }
}
