...
export class WorkflowSchedulerService {
  ...
  async scheduleTriggerChecks (): Promise<void> {
    ...
    // TODO use a queue
    const promises = triggers
      .map(trigger => this.runnerService.runWorkflowTriggerCheck(trigger, WorkflowRunStartedByOptions.trigger))
  ...
  async scheduleSleepChecks (): Promise<void> {
    ...
    // TODO use a queue
    const promises = workflowSleeps
      .map(workflowSleep => this.runnerService.wakeUpWorkflowRun(workflowSleep))
    await Promise.all(promises)
  }
}
