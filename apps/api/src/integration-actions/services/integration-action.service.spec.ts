...
describe('IntegrationActionService', () => {
  ...
  beforeEach(async () => {
    const testModule: TestingModule = await Test.createTestingModule({
      imports: [TestDatabaseModule, TypegooseModule.forFeature([IntegrationAction]), MockModule],
      providers: [IntegrationActionService],
    }).compile()

    service = testModule.get<IntegrationActionService>(IntegrationActionService)
  })
